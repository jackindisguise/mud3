 ...
 *
 * Generates a cache of blocked names from mob templates and provides
 * regex rules for catching offensive names. Used during character creation
 * to prevent players from using names that conflict with game entities or
		...
		for (const template of dungeon.templates.values()) {
			...
			if (template.display) {
				...
				if (normalizedDisplay.length > 0) {
					BLOCKED_NAMES.add(normalizedDisplay);
					blockedNameCount++;
				}
			}
