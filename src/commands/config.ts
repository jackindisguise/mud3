...
const getPromptUsage: UsageHelper = (prefix = 0) => {
	...
	const width = 20;
	const health = color("%hh (health)".padEnd(width), COLOR.CRIMSON);
	const mana = color("%mm (mana)".padEnd(width), COLOR.CYAN);
	const maxHealth = color("%HH (max health)".padEnd(width), COLOR.CRIMSON);
	const maxMana = color("%MM (max mana)".padEnd(width), COLOR.CYAN);
	const exhaustion = color("%ex (exhaustion)".padEnd(width), COLOR.LIME);
	const stamina = color("%st (stamina)".padEnd(width), COLOR.LIME);
	const xp = color("%xp (XP)".padEnd(width), COLOR.YELLOW);
	const xpToLevel = color("%XX (XP to level)".padEnd(width), COLOR.YELLOW);
	lines.push(`${_prefix}  ${health} ${mana}`);
	lines.push(`${_prefix}  ${maxHealth} ${maxMana}`);
	lines.push(`${_prefix}  ${exhaustion} ${stamina}`);
