...
Two new functions support password verification without loading the full character:
...
- `loadCharacterFromSerialized(data)` - Deserializes character data and registers it as active. Use this after successful password verification.
...
if (serialized) {
  ...
  console.log('Authenticated:', character.credentials.username);
}
